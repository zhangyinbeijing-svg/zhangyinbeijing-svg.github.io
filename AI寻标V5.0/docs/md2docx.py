# -*- coding: utf-8 -*-
"""将 AI寻标系统PRD-V5.0.md 转换为排版规范的 Word 文档"""
import os
import re
...
SRC = r'd:\项目\AI寻标\docs\AI寻标系统PRD-V5.0.md'
DST = r'd:\项目\AI寻标\docs\AI寻标系统PRD-V5.0.docx'
...
def convert():
    ...
    while i < n:
        ...
        # 图片（![alt](path)，路径相对于 md 所在目录）
        m = re.match(r'^!\[(.*?)\]\((.*?)\)$', stripped)
        if m:
            alt, rel = m.group(1), m.group(2)
            img_path = os.path.join(os.path.dirname(SRC), rel)
            if os.path.exists(img_path):
                p = doc.add_paragraph()
                p.alignment = WD_ALIGN_PARAGRAPH.CENTER
                p.add_run().add_picture(img_path, width=Cm(16))
                if alt:
                    cap = doc.add_paragraph()
                    cap.alignment = WD_ALIGN_PARAGRAPH.CENTER
                    cap.paragraph_format.space_after = Pt(8)
                    set_run(cap.add_run(alt), size=9, color=GRAY)
            else:
                p = doc.add_paragraph()
                fill_paragraph(p, f'[图片缺失：{rel}]', color=RGBColor(0xD9, 0x30, 0x25))
            i += 1
            continue
